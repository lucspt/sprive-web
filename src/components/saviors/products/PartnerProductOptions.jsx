...
import UnpublishFirstPopup from "./editor/UnpublishFirstPopup";

export default function PartnerProductOptions({ product, setName }) {
  const { unit_types, name, product_id, co2e, stages, published } = product;
  const [ _published, setPublished ] = useState(published);
  const [ showPublishPopup, setShowPublishPopup ] = useState(false);
  const [ cantPublishWarning, setCantPublishWarning ] = useState([]);
