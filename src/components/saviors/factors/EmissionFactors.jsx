...
import { fetchWithAuth, isObjectEmpty } from "../../../utils/utils"
...
export const loader = async ({ request }) => {
  ...
  if (req) {
    ...
    let response = await fetchWithAuth(`factors?${params}`)
    return response
  } else return null;
}
...
const EmissionFactors = memo(function EmissionFactors({ backButton=null, returnFactor=null }) {
  ...
  const { savior: { _id: saviorId }} = useContext(SaviorContext);
  ...
  useEffect(() => {
    if (isObjectEmpty(possibilities)) {
      fetchWithAuth(
        ...
        { setState: ({ content }) => possibilities }
      );
    };
  }, []);
